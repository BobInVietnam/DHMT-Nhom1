class BodyMapScene extends Scene {
    constructor() {
        ...
        this.hitboxes = [
            // Da (Skin) nên để ở đầu danh sách để các bộ phận khác đè lên sau
            new Hitbox("Skin", [
    ...
    checkClick() {
        // Kiểm tra ngược từ dưới lên trên để ưu tiên các bộ phận nhỏ nằm đè lên da
        for (let i = this.hitboxes.length - 1; i >= 0; i--) {
            let hb = this.hitboxes[i];
            if (hb.contains(mouseX, mouseY)) {
                bus.emit("SWITCH_SCENE", hb.name);
                return; // Thoát hàm ngay khi tìm thấy một hitbox trúng
            }
        }
    }
}